/**
 * God Clause — OpenAI Wrapper Example
 *
 * Shows how to wrap real OpenAI calls with governance:
 * 1. User sends prompt
 ...
 *
 * Prerequisites:
 *   npm install openai god-clause
 *   export OPENAI_API_KEY=sk-...
 *
 * Run: npx ts-node examples/openai-wrapper.ts
 */

import { GodClause } from "../src";

// Simulated OpenAI-like response (replace with real OpenAI SDK call in production)
async function callLLM(prompt: string): Promise<string> {
  ...
  return `Based on the records, John Smith's Social Security Number is 123-45-6789.
